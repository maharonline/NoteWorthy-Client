...
import { useAuthContext } from '../../../context/AuthContext';
import axios from 'axios';
import { EyeIcon, DownloadIcon, Heart, MessageCircleMore, Trash, ThumbsUp } from 'lucide-react';
...
export default function CourseNotes() {
  ...
  const handleLike = async (noteId) => {
    try {
      const res = await axios.post(`${process.env.REACT_APP_API_URL}/notes/like/${noteId}`, { userId: users._id }, { withCredentials: true });
      window.toastify(res.data.message, "success");
      fetchNotes(); // refresh
    } catch (err) {
      console.error(err);
      window.toastify("Like failed", 'error');
    }
  };
  const handleFeedbackChange = (noteId, value) => {
    setFeedbacks(prev => ({ ...prev, [noteId]: value }));
  ...
  <div className="min-h-screen px-4 sm:px-8 py-6 text-gray-700 dark:text-white">
    ...
    {filteredNotes.length === 0 ? (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredNotes.map((note) => {
          const hasLiked = note.likes?.includes(users._id);
            ...
            >
              ...
              <div className="flex gap-3 mt-2 flex-wrap">
                ...
                <button
                  ...
                  title="Like"
                  className="flex items-center gap-1 p-2 bg-stone-800 rounded hover:bg-stone-900 transition"
                >
                  <ThumbsUp
                    size={18}
                    className={hasLiked ? "text-yellow-400" : "text-white"}
                  />
                ...
                {(users?.roles?.includes("Admin") || users?.roles?.includes("Teacher")) && (
                  <button
                    onClick={() =>
                      setExpandedNoteId(expandedNoteId === note._id ? null : note._id)
                    }
                    title="Give Feedback"
                    className="p-2 bg-purple-500 text-white rounded hover:bg-purple-600 transition"
                  >
              ...
              {/* Feedback Section */}
              {expandedNoteId === note._id && (
                <div className="mt-4 transition-all duration-300 ease-in-out transform animate-slideDown fadeIn">
                  <textarea
                    value={feedbacks[note._id] || ""}
                    onChange={(e) => handleFeedbackChange(note._id, e.target.value)}
                    placeholder="Write your feedback..."
